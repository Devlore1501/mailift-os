...
**Bozza script AI** — nella card contenuto, "Genera bozza script" produce uno script su misura per surface/format (Reel parlato, carosello slide-per-slide, sequenza story) a partire da idea madre, angolo, hook e pillar; propone hook e CTA keyword se mancano. Ogni script segue la **struttura open loop di Mailift** (HOOK → LEAD → BODY 1 open loop → BODY 2 open loop → BODY 3 payoff → CTA): la guida di stile con esempi vive in [`backend/app/prompts/script_style.md`](backend/app/prompts/script_style.md) ed è modificabile senza toccare il codice. La bozza non viene salvata da sola: l'umano rifinisce e preme Salva (human-in-the-loop). Endpoint: `POST /api/contents/{id}/generate-script`, accessibile anche ai Contributor.
